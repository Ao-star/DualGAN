...
parser.add_argument("--dataset_name", type=str, default="sar", help="name of the dataset")
...
# print("/home/bingo05/Workspace/PyTorch-GAN-master/data/%s" % opt.dataset_name)
dataloader = DataLoader(
    ImageDataset("/home/bingo05/Workspace/PyTorch-GAN-master/data/%s" % opt.dataset_name, transforms_=transforms_),
    batch_size=opt.batch_size,
    shuffle=True,
    num_workers=opt.n_cpu,
)
val_dataloader = DataLoader(
    ImageDataset("/home/bingo05/Workspace/PyTorch-GAN-master/data/%s" % opt.dataset_name, mode="val", transforms_=transforms_),
    batch_size=16,
    shuffle=True,
    num_workers=1,
)
...
def sample_images(batches_done):
    ...
    real_B = Variable(imgs["B"].type(FloatTensor))
    fake_A = G_BA(real_B)
    # print(fake_A.shape,real_A.shape)
    # print(fake_B.shape,real_B.shape)
    AB = torch.cat((real_A.data, fake_A.data), -2)
    BA = torch.cat((real_B.data, fake_B.data), -2)
    # img_sample_ab = torch.cat((AB, BA), 0)
    save_image(AB, "images/%s/%s_ab.png" % (opt.dataset_name, batches_done), nrow=4, normalize=True)
    save_image(BA, "images/%s/%s_ba.png" % (opt.dataset_name, batches_done), nrow=4, normalize=True)
...
loss_names = ['D_A_loss','D_B_loss','D_loss','G_adv','G_cycle','G_loss']
for loss in loss_names:
    loss_dicts['train'][loss]=[]
...
for epoch in range(opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            G_adv_sum = G_adv_sum + G_adv.item()   
            G_cycle_sum = G_cycle_sum + G_cycle.item()   
            G_loss_sum = G_loss_sum + G_loss.item()

    losscol = [D_A_loss_sum,D_B_loss_sum,D_loss_sum,G_adv_sum,G_cycle_sum,G_loss_sum]
    for key,value in zip(loss_names,losscol):
        loss_dicts['train'][key].append(value)   #If an error occurs, add the tolist() operation to Append
...
with open('loss/loss.json', "w") as outfile: 
    json.dump(loss_dicts, outfile)        
